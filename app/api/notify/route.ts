...
function pillBadge(text: string, color: string) {
    ...
    return `<span style="display:inline-block;padding:3px 10px;border-radius:999px;font-size:12px;font-weight:600;${colors[color] ?? colors.purple}">${text}</span>`
}

function ctaButton(text: string, url: string) {
    return `<a href="${url}" style="display:inline-block;background:linear-gradient(135deg,#7c3aed,#4f46e5);color:white;text-decoration:none;padding:12px 28px;border-radius:10px;font-weight:600;font-size:14px;margin-top:24px">${text}</a>`
}
...
function statusChangeEmail(e: Extract<EmailEvent, { type: 'status_change' }>) {
    ...
    return emailWrapper(`
        ...
        <p style="margin:0 0 28px;color:#737373;font-size:14px">Hi ${e.requesterName}, your feature request status changed.</p>
        <div style="background:#1a1a1a;border:1px solid #262626;border-radius:12px;padding:20px;margin-bottom:24px">
          ...
          <p style="margin:0 0 16px;color:#f5f5f5;font-size:17px;font-weight:600">${e.requestTitle}</p>
          <div style="display:flex;align-items:center;gap:12px">
            <span style="color:#525252;font-size:13px;text-decoration:line-through">${formatStatus(e.oldStatus)}</span>
            <span style="color:#525252">→</span>
            ${pillBadge(formatStatus(e.newStatus), statusColors[e.newStatus] ?? 'purple')}
          </div>
        </div>
        <p style="margin:0;color:#737373;font-size:13px">Workspace: <strong style="color:#a3a3a3">${e.workspaceName}</strong></p>
...
function newCommentEmail(e: Extract<EmailEvent, { type: 'new_comment' }>) {
    return emailWrapper(`
        ...
        <p style="margin:0 0 28px;color:#737373;font-size:14px">Hi ${e.requesterName}, ${e.commenterName} left a comment on your request.</p>
        <div style="background:#1a1a1a;border:1px solid #262626;border-radius:12px;padding:20px;margin-bottom:16px">
          <p style="margin:0 0 6px;color:#a3a3a3;font-size:12px">On: <strong style="color:#d4d4d4">${e.requestTitle}</strong></p>
        </div>
        <div style="border-left:3px solid #7c3aed;padding:16px;background:#1a1a1a;border-radius:0 12px 12px 0;margin-bottom:24px">
          <p style="margin:0 0 8px;color:#a78bfa;font-size:12px;font-weight:600">${e.commenterName}</p>
          <p style="margin:0;color:#d4d4d4;font-size:14px;line-height:1.6">${e.commentText.slice(0, 300)}${e.commentText.length > 300 ? '...' : ''}</p>
        </div>
...
function newRequestEmail(e: Extract<EmailEvent, { type: 'new_request' }>) {
    return emailWrapper(`
        ...
        <p style="margin:0 0 28px;color:#737373;font-size:14px">Hi ${e.adminName}, a new request was submitted in <strong style="color:#a3a3a3">${e.workspaceName}</strong>.</p>
        <div style="background:#1a1a1a;border:1px solid #262626;border-radius:12px;padding:20px;margin-bottom:24px">
          <p style="margin:0 0 4px;color:#a3a3a3;font-size:12px">From: <strong style="color:#d4d4d4">${e.requesterName}</strong></p>
          <p style="margin:0 0 12px;color:#f5f5f5;font-size:17px;font-weight:600">${e.requestTitle}</p>
          ${e.description ? `<p style="margin:0;color:#737373;font-size:13px;line-height:1.6">${e.description.slice(0, 200)}${e.description.length > 200 ? '...' : ''}</p>` : ''}
        </div>
...
function voteMilestoneEmail(e: Extract<EmailEvent, { type: 'vote_milestone' }>) {
    return emailWrapper(`
        <div style="text-align:center;margin-bottom:28px">
          ...
          <p style="margin:0;color:#737373;font-size:14px">Hi ${e.requesterName},</p>
        </div>
        <div style="background:linear-gradient(135deg,#1e1b4b,#13111c);border:1px solid #4c1d95;border-radius:12px;padding:28px;text-align:center;margin-bottom:24px">
          ...
          <p style="margin:0;color:#c4b5fd;font-size:15px;font-weight:500">"${e.requestTitle}"</p>
        </div>
        <p style="margin:0 0 24px;color:#737373;font-size:13px;text-align:center">in <strong style="color:#a3a3a3">${e.workspaceName}</strong></p>
